...
pub enum BSideError {
    ...
    #[error("S3 failure: {0}.")]
    S3Error(String),
    #[error("The requested ressource was not found.")]
    NotFound,
...
impl IntoResponse for BSideError {
    fn into_response(self) -> Response {
        let (status, error_message) = match self {
            ...
            Self::SqlxError(_) | Self::S3Error(_) | Self::NetworkError(_) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                "An internal server error occured.".to_string()),
            Self::ConversionError(e) => {
                tracing::error!("Type conversion failed: {e}");
                (StatusCode::INTERNAL_SERVER_ERROR, "A technical error occured".to_string())
            }
        };
